...
const PACKAGES = [
  ...
  // Protocol & transport
  { name: 'oid4vp', order: 4, blurb: 'OID4VP wire format — request, response, signed JAR (RFC 9101), x509_san_dns cert helpers.' },
  { name: 'oid4vci', order: 5, blurb: 'OID4VCI client + server — Draft 13/15 normalized, PAR, DPoP both sides.' },
  { name: 'presentation-exchange', order: 6, blurb: 'DIF Presentation Exchange v2 — legacy OID4VP 1.0 query format.' },
  { name: 'dcql', order: 7, blurb: 'Digital Credentials Query Language — OID4VP 2.0 query format.' },
  // Crypto / credentials
  { name: 'jose', order: 8, blurb: 'JWS sign + verify, x5c chain validation, pluggable Signer Strategy.' },
  { name: 'sd-jwt', order: 9, blurb: 'SD-JWT-VC parser, hash binding, KB-JWT issuance / verification.' },
  { name: 'credential-format', order: 10, blurb: 'Pluggable format-handler registry (SD-JWT-VC ships; mDoc plugs in).' },
  // Trust / revocation
  { name: 'trust', order: 11, blurb: 'TrustResolver Strategy — Static, JwksUrl, SdJwtVcIssuer (.well-known/jwt-vc-issuer).' },
  { name: 'status-list', order: 12, blurb: 'IETF Token Status List + StatusResolver Strategy for revocation / suspension.' },
];
